				...
				<div class="col-md-12">
					...
					<div class="p-3 p-lg-5 border">
						<form action="{{ route('cupom.validar') }}" id="form-cupom" method="post">
							{{ csrf_field() }}
							<label for="c_code" class="text-black mb-3">Insira o código do cupom de desconto, caso tenha algum.</label>
							<div class="input-group w-75">
								<input name="codigo" type="text" class="form-control" id="c_code" placeholder="Código do cupom"
										aria-label="Coupon Code" aria-describedby="button-addon2">
								<div class="input-group-append">
									<button class="btn btn-primary btn-sm" form="form-cupom" type="submit" id="button-addon2">Validar</button>
								</div>
							</div>
						</form>
					</div>
					...
					<div class="p-3 p-lg-5 border">
						<table class="table site-block-order-table mb-5">
							...
							<tbody>
								...
								@endphp
								<tr>
									<td>{{ $produto->titulo }} <strong class="mx-2">x</strong> {{ $data->items->{$produto->id} }}</td>
								...
								<tr>
									...
									<td class="text-black">R$ {{ ((real) $subTotal * ((real) $desconto / 100)) }}</td>
								</tr>
								@endif
								<tr>
									<td class="text-black font-weight-bold"><strong>Total do Pedido</strong></td>
									<td class="text-black font-weight-bold"><strong>R$ {{ isset($total) ? $total : $subTotal }}</strong></td>
								</tr>
							</tbody>
						</table>
						...
						<div class="form-group">
							<button class="btn btn-primary btn-lg py-3 btn-block" form="form-checkout" >Finalizar Pedido</button>
						</div>
					</div>
				</div>
			</div>
