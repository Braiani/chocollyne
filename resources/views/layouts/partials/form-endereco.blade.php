	...
	<form id="form-checkout" action="{{ route('cart.checkout') }}" method="post">
		...
		<div class="form-group row">
			<div class="col-md-12">
				...
				<input type="text" class="form-control" id="nome" name="nome" value="{{ isset($user) ? $user->nome : old('nome') }}" required>
			</div>
		</div>
		...
		<div class="form-group row mb-5">
			<div class="col-md-6">
				...
				<input type="text" class="form-control" id="cpf" name="cpf" value="{{ isset($user) ? $user->cpf : old('cpf') }}" required>
			</div>
			<div class="col-md-6">
				<label for="telefone" class="text-black">Telefone <span class="text-danger">*</span></label>
				<input type="text" class="form-control" id="telefone" name="telefone" value="{{ isset($user) ? $user->telefone : old('telefone') }}" placeholder="(__) ____-____" required>
			</div>
		</div>
		...
		<div class="form-group row mb-5">
			<div class="col-md-6">
				...
				<input type="text" class="form-control" id="cep" name="cep" value="{{ isset($user) ? $user->cep : old('cep') }}" placeholder="Digite o CEP" required>
			</div>
			<div class="col-md-6">
			...
			<div class="col-md-12">
				...
				<input type="text" class="form-control" id="endereco" name="endereco" value="{{ isset($user) ? $user->endereco : old('endereco') }}" placeholder="Endereço (Rua, n, Bairro)" required>
			</div>

			<div class="col-md-4">
				<label for="numero" class="text-black">Número <span class="text-danger">*</span></label>
				<input type="text" class="form-control" id="numero" name="numero" value="{{ isset($user) ? $user->numero : old('numero') }}" placeholder="Número">
			</div>
			
			<div class="col-md-8">
				<label for="complemento" class="text-black">Complemento </label>
				<input type="text" class="form-control" id="complemento" name="complemento" value="{{ isset($user) ? $user->complemento : old('complemento') }}" placeholder="Complemento (Ex.: Apartamento 301)">
			</div>
	
			<div class="col-md-8">
				<label for="cidade" class="text-black">Cidade <span class="text-danger">*</span></label>
				<input type="text" class="form-control" id="cidade" name="cidade" value="{{ isset($user) ? $user->cidade : old('cidade') }}" required>
			</div>

			<div class="col-md-4">
				<label for="estado" class="text-black">Estado <span class="text-danger">*</span></label>
				<input type="text" class="form-control" id="estado" name="estado" value="{{ isset($user) ? $user->estado : old('estado') }}" required>
			</div>
		</div>
		...
		<div class="form-group row mb-5">
			<div class="col-md-12">
				<p class="mb-3">Crie uma conta preenchendo a informação abaixo. Já possui uma conta? <a href="{{ route('admin.home') }}">Clique aqui</a> para fazer Login.</p>
		...
		<div class="form-group">
			...
			<textarea name="observacao" id="observacao" cols="30" 
				rows="5" class="form-control" placeholder="Escreva aqui seus pedidos especiais...">{{ isset($user) ? $user->observacao : old('observacao') }}</textarea>
		</div>
	</form>
</div>
